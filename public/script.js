...
function upd(){
  ...
  const conPlaca=document.getElementById('placa-chk').checked;
  ...
  rp.style.display=conPlaca?'flex':'none';
  document.getElementById('dsf').style.width=Math.min(tg/200*100,100)+'%';
  ...
  if(!tg){p.innerHTML='<span class="pill">Ingresa grupos para ver tus beneficios</span>';return;}
  let bens=[
  ...
  p.innerHTML=bens.map(t=>`<span class="${cls(t)}">${t}</span>`).join('');
}
upd();
